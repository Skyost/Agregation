...
\begin{document}
	...
	\begin{demonstration}
		\underLine{Sens direct :} Supposons $\mathbb{K}[A] = \mathcal{C}(A)$. Le \cref{dimension-du-commutant-2} entraîne que
		\[ \deg(\mu_A) = \dim(\mathbb{K}[A]) \geq n \]
		Mais comme $\deg(\mu_A) \leq n$, on a $\deg(\mu_A) = n$. Par le théorème de Cayley-Hamilton, on conclut
		\[ \mu_A = \chi_A \]
		\underLine{Réciproque :} On suppose $\mu_A = \chi_A$. Par la \cref{dimension-du-commutant-1}, on peut trouver $x \in \in \mathbb{K}^n \setminus \{ 0 \}$ tel que $(x, Ax, \dots, A^{n-1}x)$ est une base de $\mathbb{K}^n$. Ainsi, l'application
		\[ \varphi :
		\begin{array}{cl}
