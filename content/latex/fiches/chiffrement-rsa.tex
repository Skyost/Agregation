...
\begin{document}
  ...
  \begin{proof}
    ...
    \newpar
    Soit $i$ tel que $1 \leq i \leq p-1$ et $r_i = r_k$. Montrons que $(i-k)a$ est divisible par $p$. Comme $r_i$ et $r_k$ sont respectivement le reste de la division euclidienne de $ia$ et $ka$ par $p$, on a $r_i - r_k = 0 \equiv (i-k)a \mod p$ donc $(i-k)a$ est divisible par $p$. Comme $p$ ne divise pas $a$, par le \cref{chiffrement-rsa-4}, on a $i-k$ divise $p$. Et comme $-p < i-k < p$, on en déduit que $i=k$.
    \newpar
