...
\begin{document}
  ...
  \begin{proof}
    Comme $f(x) = O \left( \frac{1}{x^2} \right)$, il existe $M > 0$ et $A > 0$ tels que
    ...
    \[ c_n(F) = \int_0^1 F(t) e^{-2i\pi n t} \, \mathrm{d}t = \int_0^1 \sum_{k=-\infty}^{+\infty} f(t+k) e^{-2i\pi n t} \, \mathrm{d}t \]
    Par convergence uniforme sur un segment, on peut échanger somme et intégrale :
    \[ c_n(F) = \sum_{k=-\infty}^{+\infty} \int_k^{k+1} f(t) e^{-2i\pi n t} \, \mathrm{d}t \]
