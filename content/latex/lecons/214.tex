...
\begin{document}
  ...
  Pour une fonction réelle $f : \mathbb{R} \rightarrow \mathbb{R}$ de classe $\mathcal{C}^1$, on sait que si $f'(x) \neq 0$ pour tout $x \in \mathbb{R}$, alors $f$ est un difféomorphisme de $\mathbb{R}$ sur $f(\mathbb{R})$ et
  \[ \forall x \in \mathbb{R}, \, (f^{-1})'(f(x)) = \frac{1}{f'(x)}. \]
  ...
  \begin{application}
    On suppose $E = \mathbb{R}^n$. Soit $\varphi : U \rightarrow \mathbb{R}^n$ un difféomorphisme de classe $\mathcal{C}^1$. Alors, $V = \varphi(U)$ est mesurable et toute fonction $f$ appartient à $L_1(V)$ si et seulement si $y \mapsto \vert \det \operatorname{Jac}(\varphi)_y \vert f (\varphi(y))$ appartient à $L_1(U)$. Dans ce cas,
