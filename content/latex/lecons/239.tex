...
\begin{document}
  ...
  \begin{corollary}
    ...
    \begin{enumerate}[label={\bfseries(\roman*)}]
      ...
      \[ |f(t,x)| \leq g_K(x) \quad \forall t \in K, \text{pp. en } x \]
    \end{enumerate}
  ...
  \begin{example}
    ...
    \begin{array}{ccc}
      ...
      x &\mapsto& \int_{0}^{+\infty} t^{x-1} e^{-t} \, \mathrm{d}t
    \end{array}
    \]
  ...
  \begin{theorem}[Dérivation sous le signe intégral]
    ...
    \begin{enumerate}[label={\bfseries(\roman*)}]
      ...
      \[ \left| \frac{\partial f}{\partial t}(t,x) \right| \leq g_K(x) \quad \forall t \in K, \text{pp. en } x \]
    \end{enumerate}
    Alors $\forall t \in I$, $x \mapsto \frac{\partial f}{\partial t}(t, x) \in L_1(X)$ et $F$ est dérivable sur $I$ avec
    \[ \forall t \in I, \, F'(t) = \int_X \frac{\partial f}{\partial t}(t, x) \, \mathrm{d}\mu(x) \]
  \end{theorem}

  \begin{remark}
    \begin{itemize}
      \item Si dans le \cref{239-2}, hypothèse (ii), on remplace ``dérivable'' par ``$\mathcal{C}^1$'', alors la fonction $F$ est de classe $\mathcal{C}^1$.
  ...
  \begin{theorem}[$k$-ième dérivée sous le signe intégral]
    ...
    \begin{enumerate}[label={\bfseries(\roman*)}]
      ...
      \[ \left| \left(\frac{\partial}{\partial t}\right)^j f(t,x) \right| \leq g_{j,K}(x) \quad \forall t \in K, \text{pp. en } x \]
    \end{enumerate}
    Alors $\forall j \in \llbracket 0, k \rrbracket$, $\forall t \in I$, $x \mapsto \left(\frac{\partial}{\partial t}\right)^j f(t,x) \in L_1(X)$ et $F \in \mathcal{C}^k(I)$ avec
    \[ \forall j \in \llbracket 0, k \rrbracket, \, \forall t \in I, \, F^{(j)}(t) = \int_X \left(\frac{\partial}{\partial t}\right)^j f(t, x) \, \mathrm{d}\mu(x) \]
  \end{theorem}
  ...
  \begin{theorem}[Théorème de Bohr-Mollerup]
  ...
  \begin{example}
    ...
    \[ \forall x \in \mathbb{R}, \, \sum_{n \in \mathbb{N}} |f_n(x)| + \sup_{t \in I} |f'_n(t)| < +\infty \]
  ...
  \begin{application}[Transformée de Fourier d'une Gaussienne]
    ...
    \[ \forall \alpha > 0, \, \forall x \in \mathbb{R}, \, \int_{\mathbb{R}} e^{-\alpha t^2} e^{-itx} \, \mathrm{d}t = \sqrt{\frac{\pi}{\alpha}} e^{-\frac{x^2}{4 \alpha}} \]
  \end{application}
  ...
  \begin{definition}
    Soient $f$ et $g$ deux fonctions de $\mathbb{R}^d$ dans $\mathbb{C}$. On dit que \textbf{la convolée} (ou \textbf{le produit de convolution}) de $f$ et $g$ en $x \in \mathbb{R}^d$ \textbf{existe} si la fonction
    \[
    \begin{array}{ccc}
  ...
  \begin{theorem}
    ...
    \begin{enumerate}[label={\bfseries(\roman*)}]
      \item Si $p, q \in [1, +\infty]$ tels que $\frac{1}{p} + \frac{1}{q} = 1$, alors $(f * g)(x)$ existe \uline{pour tout} $x \in \mathbb{R}^d$ et est uniformément continue. On a, $\Vert f * g \Vert_\infty \leq \Vert f \Vert_p \Vert g \Vert_q$ et, si $p \neq 1, +\infty$, $f * g \in \mathcal{C}_0(\mathbb{R}^d)$.
      \item Si $p = 1$ et $q = +\infty$, alors $(f * g)(x)$ existe \uline{pour tout} $x \in \mathbb{R}^d$ et $f * g \in \mathcal{C}_b(\mathbb{R}^d)$.
      \item Si $p = 1$ et $q \in [1, +\infty[$, alors $(f * g)(x)$ existe \uline{pp.} en $x \in \mathbb{R}^d$ et $f * g \in L_q(\mathbb{R}^d)$ telle que $\Vert f * g \Vert_q \leq \Vert f \Vert_1 \Vert g \Vert_q$.
      \item Si $p = 1$ et $q = 1$, alors $(f * g)(x)$ existe \uline{pp.} en $x \in \mathbb{R}^d$ et $f * g \in L_1(\mathbb{R}^d)$ telle que $\Vert f * g \Vert_1 \leq \Vert f \Vert_1 \Vert g \Vert_1$.
    \end{enumerate}
  \end{theorem}
  ...
  \begin{definition}
    ...
    \begin{enumerate}[label={\bfseries(\roman*)}]
      ...
      \item $\sup_{n \geq 1} \Vert \rho_n \Vert_1 < +\infty$.
      \item $\forall \epsilon > 0, \, \lim_{n \rightarrow +\infty} \int_{\mathbb{R}^d \setminus B(0, \epsilon)} |\rho_n(x)| \, \mathrm{d}x = 0$.
    \end{enumerate}
  \end{definition}

  \begin{remark}
    Dans la définition précédente, (i) implique (ii) lorsque les fonctions $\rho_n$ sont positives, puisque $\Vert\rho_n\Vert_1=1$. Plutôt que des suites, on pourra considérer les familles indexées par $\mathbb{R}_*^+$.
  \end{remark}
  ...
  \begin{application}[Théorème de Weierstrass]
    Toute fonction continue $f : [a,b] \rightarrow \mathbb{R}$ (avec $a, b \in \mathbb{R}$ tels que $a \leq b$) est limite uniforme de fonctions polynomiales sur $[a, b]$.
  \end{application}
  ...
  \begin{example}
    Soit $\alpha \in \mathcal{C}^\infty_K(\mathbb{R}^d)$ une densité de probabilité. Alors la suite $(\alpha_n)$ définie pour tout $n \in \mathbb{N}^*$ par $\alpha_n : x \mapsto n^d \alpha(nx)$ est régularisante.
  \end{example}
  ...
  \begin{lemma}[Lemme de Riemann-Lebesgue]
    Soit $f \in L_1(\mathbb{R}^d)$, $\widehat{f}$ existe et
    \[ \lim_{\Vert \xi \Vert \rightarrow +\infty} \widehat{f}(\xi) = 0 \]
  \end{lemma}
  ...
  \begin{theorem}[Théorème de Plancherel-Parseval]
