...
\begin{document}
  ...
  \begin{remark}
    \begin{itemize}
      \item En dimension quelconque $\mathrm{d}f_a$ dépend a priori des normes choisies sur $E$ et $F$. Cependant, en dimension finie, l'équivalence des normes implique que l'existence et la valeur de $\mathrm{d}f_a$ ne dépendent pas des normes choisies.
  ...
  \begin{proposition}
    ...
    \begin{enumerate}[label=(\roman*)]
      ...
      \item Soit $g : V \rightarrow G$. On suppose $f(U) \subseteq V$ et $g$ différentiable en $f(a)$. Alors $g \circ f$ est différentiable en $a$ et, $\mathrm{d}(g \circ f)_a = \mathrm{d}g_{f(a)} \circ \mathrm{d}f_a$.
    \end{enumerate}
  \end{proposition}
  ...
  \begin{theorem}[Inégalité des accroissements finis]
    Soit $f : U \rightarrow F$ continue sur un segment $[a,b] \subseteq U$ et différentiable sur $]a,b[$. On suppose qu'il existe $M > 0$ tel que $\VERT \mathrm{d}f_c \VERT \leq M$ pour tout $c \in ]a,b[$. Alors,
  ...
  \begin{example}
    L'application de $\mathbb{R}^2$ dans $\mathbb{R}^2$ définie par $(x, y) \mapsto (x^2-y^2, xy)$ est un difféomorphisme local de classe $\mathcal{C}^\infty$ en tout point de $\mathbb{R}^2 \setminus \{(0,0)\}$.
  \end{example}

  \reference[BMP]{9}

  \begin{application}
    Soit $\varphi : U \rightarrow \mathbb{R}^n$ un difféomorphisme de classe $\mathcal{C}^1$. Alors, $V = \varphi(U)$ est mesurable et toute fonction $f$ appartient à $L_1(V)$ si et seulement si $y \mapsto \vert \det \operatorname{Jac}(\varphi)_y \vert f (\varphi(y))$ appartient à $L_1(U)$. Dans ce cas,
    \[ \int_V f(x) \, \mathrm{d}x = \int_U \vert \det \operatorname{Jac}(\varphi)_y \vert f (\varphi(y)) \, \mathrm{d}y \]
  \end{application}
  ...
  \begin{lemma}[Morse]
    ...
    \end{itemize}
    Alors il existe un difféomorphisme $\phi = (\phi_1, \dots, \phi_n)$ de classe $\mathcal{C}^1$ entre deux voisinages de l'origine de $\mathbb{R}^n$ $V \subseteq U$ et $W$ tel que $\varphi(0) = 0$ et
  ...
  \begin{application}[Inégalité d'Hadamard]
    \[ \forall (x_1, \dots, x_n) \in (\mathbb{R}^n)^n, \, \vert \det(x_1, \dots, x_n) \vert \leq \Vert x_1 \Vert \dots \Vert x_n \Vert \]
    avec égalité si et seulement si l'un des $x_i$ est nul ou si $(x_1, \dots, x_n)$ est une famille orthogonale de $\mathbb{R}^n$.
  \end{application}
