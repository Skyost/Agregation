...
\begin{document}
  ...
  \begin{theorem}[Réduction de Jordan d'un endomorphisme nilpotent]
    On suppose que $A$ est nilpotente. Alors il existe des entiers $n_1 \geq \dots \geq n_p \geq 1$ tels que $\sum_{i=1}^p n_i=n$ et $A$ est semblable à la matrice
  ...
  \begin{theorem}[Réduction de Jordan d'un endomorphisme]
    ...
    Alors, pour tout $i \in \llbracket 1,p \rrbracket$, il existe des entiers $r_i \geq 1$ et $n_{i,1} \geq \dots \geq n_{i,r_i} \geq 1$ tels que $\sum_{j=1}^{r_i} n_{i,j} = \alpha_i$, et $A$ est semblable à
    \[ \operatorname{diag}\left(\left(J_{n_{i,j}}(\lambda_i)\right)_{\substack{1 \leq i \leq p \\ 1 \leq j \leq r_i}}\right). \]
