...
\begin{document}
  ...
  Soit $E$ un espace vectoriel de dimension finie $n$ sur un corps $\mathbb{K}$ de caractéristique différente de $2$.
  ...
  \begin{theorem}[Méthode de Gauss]
    ...
    \begin{enumerate}[label={\bfseries(\roman*)}]
      ...
      \item \underline{Sinon.} Si $q = 0$, c'est terminé. Sinon, il existe un $a_{i,j}$ non nul. On peut supposer $(i,j) = (1,2)$, on pose alors $a = a_{1,2}$. On réécrit $q$ sous la forme :
