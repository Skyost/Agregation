...
\begin{document}
  ...
  \begin{definition}
    Soit $\varphi : E \times E \rightarrow \mathbb{R}$ une application.
    \begin{itemize}
  ...
  \begin{example}
    Sur $\mathbb{R}^4$, $(x, y, z, t) \mapsto x^2 + y^2 + z^2 - t^2$ est non dégénérée (car de déterminant $-1$).
  \end{example}
  ...
  \begin{theorem}[Méthode de Gauss]
    ...
    \begin{enumerate}[label={\bfseries(\roman*)}]
      ...
      \item \uline{Sinon.} Si $q = 0$, c'est terminé. Sinon, il existe un $a_{i,j}$ non nul. On peut supposer $(i,j) = (1,2)$, on pose alors $a = a_{1,2}$. On réécrit $q$ sous la forme :
  ...
  \begin{corollary}
    ...
    \begin{enumerate}[label={\bfseries(\roman*)}]
      ...
      \item $q$ est non dégénérée si et seulement s'il existe $p \in \llbracket 0,n \rrbracket$ tel que $\operatorname{sign}(q) = (p, n-p)$.
    \end{enumerate}
  \end{corollary}
  ...
  \begin{theorem}[Classification des coniques]
    \begin{enumerate}[label={\bfseries(\roman*)}]
      ...
      \end{itemize}
      \item \uline{Si $q$ est dégénérée :} On a $ab = 0$ et $\operatorname{sign}(q) = (1,0)$ ; après avoir éventuellement échangé les coordonnées, on peut réécrire l'équation $(E)$ sous la forme
      \[ a \left( x - \frac{r}{a} \right)^2 - 2sy = h \]
      avec $h \in \mathbb{R}$.
      \begin{itemize}
        \item \uline{Si $s \neq 0$ :} $\mathcal{C}$ est une parabole.
        \item \uline{Si $s = 0$ :} si $h = 0$, $\mathcal{C}$ se réduit à la droite $x = \frac{r}{a}$ ; si $h < 0$, $\mathcal{C} = \emptyset$. Si $h > 0$, alors $\mathcal{C}$ est constituée des deux droites parallèles d'équation $x = \frac{r}{a} \pm \sqrt{\frac{h}{a}}$.
      \end{itemize}
    \end{enumerate}
  \end{theorem}
