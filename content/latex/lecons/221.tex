...
\begin{document}
  ...
  \begin{remark}[Version linéaire d'ordre $p$]
    ...
    \[ \begin{cases} Y^{(p)} = A_{p-1}(t) Y^{(p-1)} + \dots + A_0(t) Y + B(t) & \\ Y^{(k)}(t_0) = X_k & \forall k \in \llbracket 0, p-1 \rrbracket \end{cases} \]
    admet une unique solution définie sur $I$ tout entier.
  \end{remark}
  ...
  \begin{corollary}
    L'ensemble des solutions d'une équation différentielle linéaire homogène d'ordre $p$ définie sur un intervalle $I$ est un sous-espace vectoriel de $\mathcal{C}^p(I, \mathbb{K}^n)$ de dimension $np$.
  \end{corollary}
  ...
  \begin{example}
    ...
    \begin{itemize}
      ...
      \item Si $r_1 = r_2$, $\mathcal{S}_H = \{t \mapsto (\lambda t + \mu) e^{r_1 t} \mid \lambda, \mu \in \mathbb{C}\}$.
    \end{itemize}
  \end{example}
  ...
  \begin{theorem}[Théorème de Floquet]
  ...
  \begin{theorem}[Théorème de Massera]
  ...
  \begin{corollary}[Équations de Ricatti]
    ...
    \[ (R) \iff z' = (2a(t) \varphi_0(t) + b(t))z + a(t)z^2 \]
