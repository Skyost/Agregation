...
\begin{document}
  ...
  \begin{theorem}
    Soient $U$ un ouvert de $\mathbb{R}^n$ et $\varphi : U \to \mathbb{R}^n$ un $\mathcal{C}^1$-difféomorphisme. On pose $V = \varphi(U)$. Alors $V$ est un ouvert de $\mathbb{R}^n$. De plus, pour toute fonction mesurable $f : V \to \mathbb{R}$, on a
    \[
      f \in L^1(V)
      \iff
      \left(y \mapsto
      f(\varphi(y)) \left| \det \operatorname{Jac}\varphi(y) \right|\right)
      \in L^1(U).
    \]
    Dans ce cas,
    \[
      ...
      =
      \int_U f(\varphi(y))
      \left| \det \operatorname{Jac}\varphi(y) \right| \, \mathrm{d}y.
    \]
  \end{theorem}
  ...
  \begin{example}
    \[ \sum_{n=0}^{+\infty} \frac{(-1)^n}{3n+1} = \int_0^1 \frac{\mathrm{d}x}{x^3 + 1} = \frac{3\ln(2) + \sqrt{3}\pi}{9} \]
  \end{example}
