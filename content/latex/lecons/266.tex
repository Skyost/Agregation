...
\begin{document}
  ...
  \begin{proposition}
    ...
    \[ A \text{ et } B \text{ sont indépendants} \iff \mathbb{P}_B(A) = \mathbb{P}(A) \iff \mathbb{P}_A(B) = \mathbb{P}(B) \]
  \end{proposition}
  ...
  \begin{example}
    On considère deux gènes $a$ et $b$ tels que la redondance de l'un d'entre eux entraîne l'acquisition d'un caractère $\mathcal{C}$. Anselme et Colette possèdent chacun la combinaison $ab$ et attendent un enfant : ils lui transmettront chacun, indépendamment, soit le gène $a$, soit le gène $b$, avec la même probabilité $\frac{1}{2}$. On considère les événements :
    \begin{itemize}
      \item $A$ : Colette transmet le gène $a$.
      \item $B$ : Anselme transmet le gène $b$.
      \item $C$ : l'enfant présente le caractère $\mathcal{C}$.
    \end{itemize}
  ...
  \begin{definition}
    On dit qu'une famille de sous-tribus $(\mathcal{A}_i)_{i \in I}$ de $\mathcal{A}$ est \textbf{indépendante} (sous $\mathbb{P}$) si
  ...
  \begin{proposition}
    Soient $X$ et $Y$ deux variables aléatoires réelles indépendantes et intégrables. Alors,
  ...
  \begin{definition}
    ...
    \[ \operatorname{Covar}(X,Y) = \mathbb{E}\left((X - \mathbb{E}(X))(Y - \mathbb{E}(Y))\right) = 0 \]
  \end{definition}
