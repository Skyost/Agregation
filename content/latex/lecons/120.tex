...
\begin{document}
  ...
  \begin{definition}
    \textbf{L'indicatrice d'Euler} $\varphi$ est la fonction qui, à un entier $k$, associe le nombre d'entiers compris entre $1$ et $k$ qui sont premiers avec $k$.
  \end{definition}
  ...
  \begin{corollary}
    $\forall m, n \in \mathbb{N}^*$ premiers entre eux,
  ...
  \begin{proposition}[Petit théorème de Fermat]
    Pour tout entier relatif $a$ et tout nombre premier $p$ tels que $p \nmid a$, on a $a^{p-1} \equiv 1 \mod p$.
  \end{proposition}
  ...
  \begin{definition}
    Afin de chiffrer un \textbf{message} (tout entier découpé en séquence d'entiers de taille bornée) en utilisant RSA, on choisit deux nombres premiers distincts $p$ et $q$, puis on pose $n=pq$ :
    \begin{itemize}
      \item On choisit $e$ premier avec $\varphi(n)$ et on note $d$ son inverse modulo $\varphi(n)$.
      \item La \textbf{clé publique} est $(n,e)$ et la \textbf{clé privée} est $(n,d)$ ; les facteurs $p$ et $q$ doivent rester secrets.
    \end{itemize}
  \end{definition}
  ...
  \begin{theorem}[Chiffrement RSA]
    ...
    \begin{enumerate}[label=(\roman*)]
      \item Possédant la clé publique, on peut \textit{chiffrer} ce message en un message $m'$, dont on note $m_1', \dots, m_r$ les coordonnées :
      \[ \forall i \in \llbracket 1,r \rrbracket, \, m_i' \equiv m_i^e \mod n, \qquad 0 \leq m_i' < n \]
      \item Possédant la clé privée, on peut \textit{déchiffrer} le message $m'$ pour reconstituer $m$ :
      \[ \forall i \in \llbracket 1,r \rrbracket, \, (m_i')^d \equiv m_i \mod n \]
    \end{enumerate}
  \end{theorem}
