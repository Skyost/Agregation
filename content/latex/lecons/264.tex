...
\begin{document}
  ...
  \begin{definition}
    \begin{itemize}
      \item On dit qu'une loi $\mu$ est \textbf{discrète} s'il existe un ensemble $D$ fini ou dénombrable tel que $\mu(D) = 1$.
  ...
  \begin{example}
    On pose $\Omega=\{0,1\}^{\mathbb{N}}$ et, pour $\omega=(\omega_n)_{n\in\mathbb{N}}\in\Omega$,
    \[ X(\omega)=\inf\{n\in\mathbb{N}\mid \omega_n=0\} \]
    (avec la convention $\inf\varnothing=+\infty$). Alors $X$ est une variable aléatoire discrète, à valeurs dans $\mathbb{N}\cup\{+\infty\}$.
  \end{example}
  ...
  \begin{remark}
    Si $D$ est un ensemble fini ou dénombrable et $(p_i)_{i \in D}$ est une famille de réels positifs de somme égale à $1$, alors en posant $\Omega = D$, $\mathcal{A} = \mathcal{P}(D)$, $X : \omega \mapsto \omega$ et $\mathbb{P} = \sum_{i \in D} p_i \delta_i$, on a construit une variable aléatoire discrète $X$ sur $(\Omega, \mathcal{A}, \mathbb{P})$.
  \end{remark}
  ...
  \begin{proposition}
    ...
    \[ \forall (x_1,\dots,x_n) \in \prod_{j=1}^n X_j(\Omega), \, \mathbb{P}(X_1 = x_1, \dots, X_n = x_n) = \prod_{j = 1}^n \mathbb{P}(X_j = x_j) \]
  \end{proposition}
  ...
  \begin{theorem}
    Soient $X_1$ et $X_2$ deux variables aléatoires indépendantes et intégrables. Alors,
  ...
  \begin{definition}
    On dit que $X$ \textbf{admet un moment d'ordre $2$} si elle est de carré intégrable, ie. $X^2 \in \mathcal{L}_1$. On note $\mathcal{L}_2(\Omega, \mathcal{A}, \mathbb{P})$ (ou simplement $\mathcal{L}_2(\Omega)$ voire $\mathcal{L}_2$ s'il n'y a pas d'ambiguïté) l'espace des variables aléatoires de carré intégrable.
  \end{definition}
  ...
  \begin{definition}
    ...
    \begin{itemize}
      ...
      \[ \operatorname{Var}(X_1) = \operatorname{Covar}(X_1, X_1) = \mathbb{E}\left((X_1 - \mathbb{E}(X_1))^2\right) = \mathbb{E}(X_1^2) - (\mathbb{E}(X_1))^2  \]
    \end{itemize}
  \end{definition}
  ...
  \begin{example}
    \begin{itemize}
      \item $\operatorname{Var}(\mathbb{1}_A) = \mathbb{P}(A)(1-\mathbb{P}(A))$.
  ...
  \begin{theorem}[Théorème de Lévy]
  ...
  \begin{application}[Théorème des événements rares de Poisson]
    Soit $(N_n)_{n \geq 1}$ une suite d'entiers tendant vers l'infini. On considère des événements $A_{n,1}, \dots, A_{n,N_n}$ indépendants avec $\mathbb{P}(A_{n,k}) = p_{n,k}$. On suppose également que :
    \begin{enumerate}[label={\bfseries(\roman*)}]
  ...
  \begin{theorem}[Théorème de Moivre-Laplace]
  ...
  \begin{example}
    Si chaque seconde, il y a une probabilité $p = \frac{1}{600}$ qu'un client entre dans un magasin, le nombre de clients qui entrent sur un intervalle d'une heure suit approximativement une loi de Poisson de paramètre $\lambda = 3600p = 6$.
    \newpar
