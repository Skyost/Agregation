...
\begin{document}
  ...
  \begin{proposition}
    \begin{enumerate}[label={\bfseries(\roman*)}]
      ...
      \item Si $f$ est $n \geq 2$ fois dérivable en $0$, $f'$ admet un développement limité en $0$ : $f'(x) = \sum_{k=1}^{n} a_k x^{k-1} + o(x^{n-1})$.
      \item Si $f$ est dérivable sur $I$ si $f'$ admet un développement limité en $0$ : $f'(x) = \sum_{k=0}^{n} a_k x^k + o(x^n)$ ; alors, $f$ admet un développement limité en $0$ donné par $f(x) = f(0) + \sum_{k=0}^{n} \frac{a_k}{k+1} x^{k+1} + o(x^{n+1})$.
  ...
  \begin{cexample}
    ...
    Alors $g$ est $\mathcal{C}^\infty$, vérifie $g^{(n)}(0) = (-1)^n (n!)^2$ pour tout entier $n$, et $\sum \frac{g^{(n)}(0)}{n!} z^n$ a un rayon de convergence nul.
  \end{cexample}
