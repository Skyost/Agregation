...
\begin{document}
  ...
  \begin{proposition}
    \begin{enumerate}[label={\bfseries(\roman*)}]
      ...
      \begin{itemize}
        ...
        \item $(gh)' = g'h + gh'$.
        \item $\left( \frac{g}{h} \right)' = \frac{g'h - gh'}{h^2}$ quand $h$ ne s'annule pas sur $\Omega$.
      \end{itemize}
  ...
  \begin{definition}
    ...
    \[ D(a, r) \subseteq \Omega \text{ et } \forall z \in D(a, r), \, f(z) = \sum_{n=0}^{+\infty} a_n (z-a)^n \]
  ...
  \begin{proposition}
    Soit $\sum a_n z^n$ une série entière de rayon de convergence $r \neq 0$. On note $f$ la somme de cette série sur $D(0,r)$. Alors $f \in \mathcal{A}(D(0, r))$ et, si $\vert z - a \vert < r - \vert a \vert$ :
    \[ f(z) = \sum_{k=0}^{+\infty} \frac{S^{(k)}(a)}{k!} (z-a)^k \]
    (où $f^{(k)}$ désigne la $k$-ième dérivée complexe de $f$).
  \end{proposition}
  ...
  \begin{corollary}
    Si $\Omega$ est connexe, $\mathcal{A}(\Omega)$ est une algèbre intègre.
  \end{corollary}

  \reference{53}

  \begin{remark}[Prolongement analytique]
    Reformulé de manière équivalente au \cref{245-1}, si $\Omega$ est connexe et si deux fonctions analytiques coïncident sur un sous-ensemble de $\Omega$ qui possède un point d'accumulation dans $\Omega$, alors elles sont égales sur $\Omega$.
  \end{remark}
  ...
  \begin{definition}
    Soit $\gamma : [a,b] \rightarrow \Omega$ une courbe fermée telle que $\omega \notin \gamma^*$. \textbf{L'indice} de $\omega$ par rapport à $\gamma$, noté $I(\omega, \gamma)$, est défini par
    \[ I(\omega, \gamma) = \frac{1}{2i\pi} \int_\gamma \frac{1}{z-\omega} \, \mathrm{d}z = \frac{1}{2i\pi} \int_a^b \frac{\gamma'(t)}{\gamma(t)-\omega} \, \mathrm{d}t \]
  \end{definition}
  ...
  \begin{corollary}
    On a $\mathcal{H}(\Omega) \subseteq \mathcal{A}(\Omega)$. De plus, si $a \in \Omega$ et si $r \in ]0, d(a,\mathbb{C}\setminus\Omega)[$, on a
    \[ f(a + h) = \sum_{n=0}^{+\infty} a_n h^n \text{ pour } \vert h \vert < r \text{ avec } a_n = \frac{f^{(n)}(a)}{n!} = \frac{1}{2i\pi} \int_{C^+(a,r)} \frac{f(z)}{(z-a)^{n+1}} \, \mathrm{d}z \]
  \end{corollary}
  ...
  \begin{theorem}[Principe du maximum]
    On suppose $\Omega$ borné et $f$ holomorphe dans $\Omega$ et continue dans $\overline{\Omega}$. On note $M$ le $\sup$ de $\vert f \vert$ sur la frontière (compacte) de $\Omega$. Alors,
  ...
  \begin{application}
    Soit $f \in L_1(\mathbb{R})$. Si sa transformée de Fourier $\widehat{f} : x \mapsto \int_{\mathbb{R}} f(t) e^{-ixt} \, \mathrm{d}t$ est nulle, alors $f = 0$.
  \end{application}
  ...
  \begin{example}
    ...
    \[
      ...
      \end{array}
    \]
    se prolonge en une fonction méromorphe sur $\mathbb{C}$, dont les pôles sont les entiers négatifs.
  \end{example}
  ...
  \begin{application}[Théorème de Rouché]
    ...
    \[ z \in \gamma^* \implies \vert h(z) \vert < \vert g(z) \vert \]
    Alors,
    \[ Z(g) = Z(g + h) \]
  \end{application}
